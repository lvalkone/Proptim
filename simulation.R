...
# Get demographics data and create the population -------------------------
...
for (i in 1:nrow(agl)) {
  population %<>% add_row(
    a = rep(agl[i, "agegroups"], agl[i, "count"]),
...
subscriptions <- SCM$new("subscriptions",
  uflist = list(
    # Purchase time
    utime = "n : global_i", # from global variable
    # Customer id
    uid = "n : global_id", # from global variable
    # Customer id (original i.e. will not be scaled)
    uido = "n : global_id", # from global variable
    # Unobserved noise in reference price
    urp = function(n) {
      population[global_id, "urp"]
    }
  ),
  vflist = list(
    ...
    x1 = function(utime) {
      ...
      if (utime[global_i] > 18) {
        price1 <- (p1 + p_inc * 2)
        price1
      }
      if (utime[global_i] > 24) {
        price1 <- X_prices
      }
      price1
    },
    ...
    p = function(uid, x1, tp1, prds) {
      plogis(b_rpx * (tp1 - x1) + b_prds * log(prds + 1) + b_first * (prds == 0))
    },
    # Purchase choice
    y = function(uid, p) {
      rbinom(length(uid), 1, p)
    }
  )
)
...
global_id <- sample(population$id, size = n, replace = FALSE)
...
# Following rounds (periods t = 1,2,...,M) --------------------------------
...
for (i in 2:M) {
  ...
  global_i <- i
  # Collect id's
  global_id <- c(customers_id, non_customers_id)
...
pricesC <- seq(current_price - 5, current_price + 5, by = 0.5)

# Sample a set for the conjoint of current customers:
id_current_cust <- data_subs %>%
  filter(utime == 24 & y == 1) %>%
  select(uid) %>%
  pull()

cj_subpop_subs <- population %>%
  filter(id %in% id_current_cust) %>%
  sample_n(size = n_units, replace = FALSE)
...
cj_subpop_nons <- population %>%
  filter(id %in% id_non_cust) %>%
  sample_n(size = n_units, replace = FALSE)

# Sample a set for the conjoint of never been customers:
id_non_cust_never <- population %>%
  filter(!id %in% union(id_current_cust, id_non_cust)) %>%
  select(id) %>%
  pull()

cj_subpop_nons_never <- population %>%
  filter(id %in% id_non_cust_never) %>%
  sample_n(size = n_units, replace = FALSE)
...
conjoint_sim_nons_never <- SCM$new("conjoint_sim_nons_never",
  ...
  vflist = list(
    ...
    p = function(uid, x1, tp1, prds, k) {
      plogis(b_rpx * (tp1 + k - x1) + b_prds * log(prds + 1) + b_first * (prds == 0))
    },
    # Choice
    y = function(uid, p) {
      rbinom(length(uid), 1, p)
...
# Add a column including information of price combinations to be shown
cj_subpop_nons$ux <- as.vector(
  replicate(n = n_units, expr = sample(1:length(pricesC), kC, replace = FALSE))
)
...
pop_H1$x1 <- NA
pop_H1$y <- NA # initialize
pop_H1$utime <- global_i # update time
...
data_H0_1 <- data_subs %>%
  filter(uid %in% non_customers_id1) %>%
  group_by(uid) %>%
  slice_tail()

# Select variables needed in new purchase simulations
data_H0_1 %<>%
  select(uid, uido, a, g, l, prds) %>%
  as.data.frame()
...
data_H0_2 <- population %>%
  filter(population$id %in% non_customers_id2) %>%
  as.data.frame()
...
# Save data to be used in modeling
save(data_subs, data_subsC, data_nonsC, data_nonsC_never, id_collector_subs, id_collector_nonsubs, population, N, file = "cabc_data.RData")


# Future purchase simulations (t = 25) ------------------------------------
...
for (j in 1:length(X_prices_list)) {
  ...
  p0 <- data_t25 %>%
    filter(uido %in% pop_H0$uido) %>%
    select(p) %>%
    unlist() %>%
    mean()
  p1 <- data_t25 %>%
    filter(uido %in% pop_H1$uido) %>%
    select(p) %>%
    unlist() %>%
    mean()
